...
Houses the PersonFollower class: the top-level public controller that produces a
forward/back velocity command (a three-zone cruise/stop/brake on the target
standoff distance) and a yaw command (PID on the bearing error that re-centers
the person in frame), including depth/LiDAR fusion, gait estimation, and
lost-target re-acquisition.
"""
