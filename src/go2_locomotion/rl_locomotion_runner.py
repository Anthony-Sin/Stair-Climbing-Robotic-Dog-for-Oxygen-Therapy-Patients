"""rl_sar Go2 robot_lab policy runner (the ``blind_rl`` climb backend).

Split out of ``rl_locomotion_policy`` (structural move, behaviour-preserving).
The ``RLLocomotionPolicy`` class is a single irreducible unit and is moved whole;
the ``rl_locomotion_policy`` facade re-exports it plus ``get_dof_names`` so the
...
from go2_locomotion.rl_locomotion_contract import (
    GO2_CALF_LEN_M,
    GO2_THIGH_LEN_M,
