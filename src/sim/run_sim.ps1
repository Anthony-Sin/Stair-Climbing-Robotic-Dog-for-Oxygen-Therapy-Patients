...
Set-Content -LiteralPath $SummaryLog -Encoding UTF8 -Value @(
    ...
    "  Docker will not command the robot until both TensorRT engine files exist under src/sim/models/.",
...
function Resolve-HostRuntimePath {
    ...
    if ($normalized -match '^\\models\\(.+)$') {
        return Join-Path (Join-Path $SrcRoot "sim\models") $matches[1]
    }
...
function Test-ModelPreflight {
    ...
    if ($missing.Count -eq 0) {
        Write-Stage "models" "ready" "Required TensorRT engine files are present" @{
            model_path = (Join-Path $SrcRoot "sim\models")
        }
        return $true
    }
...
function Invoke-PerfTracker {
    $perfScript = Join-Path $SrcRoot "perf_tracker\update_table.py"
...
if ($NoIsaac) {
    ...
    $IsaacWindowScript = Join-Path $SrcRoot "sim\run_isaac_window.ps1"
...
Write-DonePath "perf"    (Join-Path $SrcRoot 'perf_tracker\data\performance_table.csv') "muted"
