...
param(
    ...
    # top landing, 6.77 = centre of the 1.0 m landing, 0.5 m from the far edge). Only set
    # this to override; the top X does not change with riser height (only Z changes).
    [double]$WaypointX = 0,
    ...
    # Isaac auto-exits early if the robot reaches the waypoint (clean climb) or falls -- the cap
    # is the safety net for runs that wedge / stall without triggering either exit condition.
...
foreach ($h in $Heights) {
    ...
    $wpX = if ($WaypointX -gt 0) { $WaypointX } else { 6.77 }
    Write-Host ("   14 steps -> top landing {0:N2} m high  |  top edge x=6.27 m, waypoint x={1:N2} m" -f $topZ, $wpX) -ForegroundColor Cyan
    Write-Host "=================================================================" -ForegroundColor Cyan
    ...
    $simArgs = @{
        ...
        # Spawn 1 m before the stair base (x=2.0) instead of the default far-start x=-4.5.
        # At ~3% realtime the 5-min wall cap = ~9 sim-sec; the robot only reached x=0.4
        # from -4.5 before the cap cut every prior sweep run. From x=1.0 it reaches the
        # first riser in ~2 sim-sec and can actually attempt the climb.
        Go2X                = 1.0
    }
...
#   python src\sim\analysis\sweep_present.py --manifest log\stair_sweep_<stamp>\manifest.json
if (-not $NoPresentation) {
    try {
        ...
        Write-Host ("  (regen anytime: python src\sim\analysis\sweep_present.py --manifest `"{0}`")" -f $manifestPath) -ForegroundColor DarkGray
    } catch {
