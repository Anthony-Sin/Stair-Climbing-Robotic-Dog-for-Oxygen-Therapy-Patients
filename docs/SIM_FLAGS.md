...
- **Launcher**: [sim/run_sim.ps1](file:///c:/Users/antho/Downloads/Stair-Climbing-Robotic-Dog-for-Oxygen-Therapy-Patients/src/sim/run_sim.ps1)
- **Isaac CLI args**: [sim/isaac/isaac_args.py](file:///c:/Users/antho/Downloads/Stair-Climbing-Robotic-Dog-for-Oxygen-Therapy-Patients/src/sim/isaac/isaac_args.py)
- **Isaac Sim Env**: [sim/isaac/isaac_env.py](file:///c:/Users/antho/Downloads/Stair-Climbing-Robotic-Dog-for-Oxygen-Therapy-Patients/src/sim/isaac/isaac_env.py)
- **Docker Controller**: [core/args_parser.py](file:///c:/Users/antho/Downloads/Stair-Climbing-Robotic-Dog-for-Oxygen-Therapy-Patients/src/core/args_parser.py)

---

## 🚀 Common Launch Recipes

Run these commands from the project root using `.\src\sim\run_sim.bat`:
- **Standard (GUI)**: `.\src\sim\run_sim.bat` (Starts GUI and Docker controller automatically)
- **Headless & Fast**: `.\src\sim\run_sim.bat --headless --fast-render` (Fastest for iteration/CI)
- **Warm Boot**: `.\src\sim\run_sim.bat --headless --warm` (Keeps Isaac alive across runs to bypass boot lag)
- **Warm Shutdown**: `.\src\sim\run_sim.bat --warm-shutdown` (Cleans up the background warm process)
- **Locomotion Self-Test**: `.\src\sim\run_sim.bat --self-test-walk` (Test robot locomotion without Docker)
- **Stair Waypoint Test**: `.\src\sim\run_sim.bat --stair-waypoint-test` (Test stair climbing without Docker controller)
- **Sim-to-Real Validation**: `.\src\sim\run_sim.bat --sim2real-validation-cam` (Applies camera/LiDAR/IMU noise)
...
- **Run the whole battery**: `.\src\sim\run_bench.bat`
- **Subset / reorder**: `.\src\sim\run_bench.bat --only stairs_steep,ramp_20deg`
- **Dry run**: `.\src\sim\run_bench.bat --dry-run` (lists battery without booting)
...
```powershell
& "$env:ISAACSIM_DIR\python.bat" src/tests/test_robot_simulation.py
```
...
```powershell
& "$env:ISAACSIM_DIR\python.bat" src/sim/isaac/isaac_env.py --headless --verification-image src/tests/test_robot_simulation.png --exit-after-verification
```
