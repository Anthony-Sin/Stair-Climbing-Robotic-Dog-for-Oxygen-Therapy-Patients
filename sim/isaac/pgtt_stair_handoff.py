...
class HandoffConfig:
    ...
    climb_max_sec: float = 20.0          # max continuous climb before handing back (parkour climbs
                                         # CONTINUOUSLY for this long; the 6s default cycled it 14x and
                                         # thrashed the gain-swap -- let the trained net run instead)
        ...
        *,
        ...
        if self.state == "climb":
            ...
            if done or timeout or abort:
                reason = "riser_climbed" if done else ("aborted_tilt" if abort else "timeout")
                self.state = "walk"
                self._cooldown_until = float(now) + float(self.cfg.re_eval_cooldown_sec)
                self.stall.reset()
                if done:
