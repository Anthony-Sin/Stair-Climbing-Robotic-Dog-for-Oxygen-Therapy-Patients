import sys
import os

try:
    from isaacsim import SimulationApp
except ImportError:
    from omni.isaac.kit import SimulationApp

simulation_app = SimulationApp({"headless": True})

from pxr import Usd, UsdGeom
import omni.usd
import omni.kit.app

try:
    from omni.isaac.core.utils.stage import add_reference_to_stage
except ModuleNotFoundError:
    from isaacsim.core.utils.stage import add_reference_to_stage

def main():
    fixed_go2_usd = r"c:\Users\antho\Downloads\Stair-Climbing-Robotic-Dog-for-Oxygen-Therapy-Patients\isaac\assets\go2_fixed\go2.usd"
    
    stage = omni.usd.get_context().get_stage()
    GO2_USD_PATH = "/World/Go2"
    
    add_reference_to_stage(usd_path=fixed_go2_usd, prim_path=GO2_USD_PATH)
    
    go2_prim = stage.GetPrimAtPath(GO2_USD_PATH)
    
    vsets = go2_prim.GetVariantSets()
    vsets.GetVariantSet("Physics").SetVariantSelection("physx")
    vsets.GetVariantSet("Sensor").SetVariantSelection("Sensors")
    vsets.GetVariantSet("Robot").SetVariantSelection("Robot")
    
    # Check payload load status before and after loading
    print("\n--- Load status BEFORE stage.Load ---")
    print(f"IsLoaded: {go2_prim.IsLoaded()}")
    
    # Load
    stage.Load(GO2_USD_PATH)
    
    for _ in range(10):
        omni.kit.app.get_app().update()
        
    print("\n--- Load status AFTER stage.Load ---")
    print(f"IsLoaded: {go2_prim.IsLoaded()}")
    
    # Print the load rules of the stage
    print(f"Stage load rules: {stage.GetLoadRules()}")
    
    # Check all prims and see if any have payload metadata that is not loaded
    for prim in Usd.PrimRange(go2_prim):
        if prim.HasPayload():
            print(f"Payload Prim: {prim.GetPath()}, IsLoaded: {prim.IsLoaded()}")
            
    simulation_app.close()

if __name__ == "__main__":
    main()
