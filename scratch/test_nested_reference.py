...
def main():
    ...
    # First add reference to the main go2 robot at /World/Go2
    add_reference_to_stage(usd_path=go2_usd, prim_path=GO2_USD_PATH)
    
    # Create prims under /World/Go2 and add explicit references to base_usd's /visuals and /meshes
    ...
    # Load the stage paths
    stage.Load(GO2_USD_PATH)
    stage.Load(GO2_USD_PATH + "/visuals")
    stage.Load(GO2_USD_PATH + "/meshes")
    
    # Let's wait a few steps for asynchronous payload rendering if needed
    for _ in range(30):
        omni.kit.app.get_app().update()
        
    # Check meshes
    print(f"\nMeshes under {GO2_USD_PATH}: {count_meshes(stage, GO2_USD_PATH)}")
    print(f"Meshes under {GO2_USD_PATH}/visuals: {count_meshes(stage, GO2_USD_PATH + '/visuals')}")
    print(f"Meshes under {GO2_USD_PATH}/meshes: {count_meshes(stage, GO2_USD_PATH + '/meshes')}")
